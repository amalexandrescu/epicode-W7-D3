...
cartButton.addEventListener("click", () => {
  ...
  // divParent.innerHTML = "";
  // divParent.innerHTML = `<a class="dropdown-item text-center bg-info font-weight-bold" href="#">
  //   ${theCart.length} books in the cart
  // </a>
  // <div class="d-flex justify-content-center" style="width:100%">
  // <button id="empty" type="button" class="btn btn-warning my-3">Empty cart</button>
  // </div>`;
  ...
  for (product of theCart) {
    ...
    <div class="d-flex justify-content-center" style="width:100%">
    <button id="remove" type="button" class="btn btn-danger my-3">Remove</button>
    </div>
    </div>`;
  }
});
...
//   const currentSearch = array.filter((element) => {
//     if (element.includes(`${inputField.value}`)) console.log(element);
// array.forEach((el) => {
//   if (el.includes(`${inputField.value}`)) {
//     // do something
//     createCard(el);
//   }
// });
...
emptyCartButton.addEventListener("click", () => {
  ...
  for (let el of divsToRemove) el.remove();
...
const getTitles = async () => {
  ...
  //     if (element.includes(`${inputField.value}`)) console.log(element.title);
  //   });
  // }
  // }
  ...
  //   if (inputField.value.length >= 3) {
  //     result.filter((element) => {
  //       if (element.title.includes(`${inputField.value}`)) {
  //         // createCard(element);
